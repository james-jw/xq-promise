...
declare %unit:test function test:callbacks-add() {
  ...
  return
    ...
    let $work := promise:defer(trace(?, 'Hello world!'))
...
declare %unit:test function test:callback-then() {
  let $greet := function ($res) { 'Hello ' || $name || '!'}
  let $promise := 
...
declare %unit:test function test:when-with-same-arity() {
  let $greet := function ($greeting, $name) { $greeting || ' ' || $name || '!'}
  let $promise := 
    promise:when((
...
declare %unit:test function test:fork-join() {
  ...
  return
    let $out := promise:fork-join($promises, map {
    let $sum := sum($out)
    return
...
declare %unit:test function test:fork-join-shared-resource() {
  let $shared := map ( for $i in 1000 return map { 'value':$i })
  let $work := function ($input) { 
    let $value := $shared?*[. = $input]
    return 
      element {text} {json:serialize($input)} 
  }
  let $promises := for $i in (1 to 1000) 
                   return promise:defer($work, $i)
  return
    let $out := promise:fork-join($promises)
    let $sum := sum(($nodes ! json:parse(.))?value)  
    return
      unit:assert-equals($sum, 500500)
}
